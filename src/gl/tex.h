...
* @Last Modified time: 2017-04-05 23:15:41
*/
#ifndef _GL_TEX_
#define _GL_TEX_
  ...
  public:
	...
	void update_from_matrix(Eigen::MatrixXf &m, GLint fmt, NVGcontext *nvg, size_t iw = 0, bool weights = false) {
		...
		for ( size_t i = 0; i < data_size; i++ ) {
			...
			r = float_image.block(0, 0, 1, 1024).cast<uint32_t>(); r.resize(image_size, image_size); r = r.unaryExpr([](uint32_t x) {return x << 0;});
			g = float_image.block(1, 0, 1, 1024).cast<uint32_t>(); g.resize(image_size, image_size); g = g.unaryExpr([](uint32_t x) {return x << 8;});
			b = float_image.block(2, 0, 1, 1024).cast<uint32_t>(); b.resize(image_size, image_size); b = b.unaryExpr([](uint32_t x) {return x << 16;});
	...
	void load(std::deque<datapoint>& data, GLint fmt, NVGcontext *nvg, size_t iw = 0) {
		...
		if (fmt == GL_RED) {
			...
			id = (GLuint)nvgCreateImageA ( nvg, sqr_dim * image_size, sqr_dim * image_size, NVG_IMAGE_NEAREST, ( unsigned char * ) rgba_image.data() );
