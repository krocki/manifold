...
class Texture {
  ...
  public:
	...
	void update_from_matrix ( Eigen::MatrixXf &m, GLint fmt, NVGcontext *nvg, size_t iw = 0, bool weights = false, bool normalize = false );
...
std::map<int, Texture*> textures_data;

//TODO: clean up
// put 4 functions into 1

void Texture::update_from_matrix ( Eigen::MatrixXf &m, GLint fmt, NVGcontext *nvg, size_t iw, bool weights, bool normalize ) {
	...
	if ( fmt == GL_RED ) {
		...
		if ( allocated )
			...
			id = ( GLuint ) nvgCreateImageRGBA ( nvg, sqr_dim * image_size, sqr_dim * image_size, NVG_IMAGE_NEAREST, ( unsigned char * ) rgba_image.data() );

			textures_data[id] = this;

		}

		if ( save_png )
			stbi_write_png ( string_format ( "tex/%04d_" + return_current_time_and_date ( "%y%m%d_%H%M%S" ) + ".png"  , id ).c_str(), sqr_dim * image_size, sqr_dim * image_size, 4, rgba_image.data(),
			                 sqr_dim * image_size * 4 );
...
void Texture::load ( std::deque<datapoint> &data, GLint fmt, NVGcontext *nvg, size_t iw) {
