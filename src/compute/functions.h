...
template <class FX>
void generate_ndims ( size_t n_dims, FX fx, Eigen::MatrixXf &points, size_t N = 1, gen_type gtype = INDEPENDENT ) {

	points.resize ( n_dims, N );
	
	switch ( gtype ) {
	
		case INDEPENDENT:
			for ( size_t i = 0; i < N; i++ ) {
				for ( size_t n = 0; n < n_dims; n++ )
				
					points ( n, i ) = rng.get ( fx );
