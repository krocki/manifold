...
* @Last Modified time: 2017-04-18 20:48:45
*/
...
class NN {
  ...
  public:
	...
	NN ( size_t minibatch_size, float _decay = 0.0f, float _learning_rate = 1e-4, network_type type = MLP,
	     std::vector<int> _layer_sizes = {}, nonlinearity_type _ltype = SIGMOID, bool _apply_dropout = false ) :
		batch_size ( minibatch_size ), decay ( _decay ), learning_rate ( _learning_rate ), ntype ( type ), use_dropout(_apply_dropout) {
	...
	void init_net ( nonlinearity_type ltype ) {
		...
		for ( size_t l = 0; l < layer_sizes.size() - 1; l++ ) {
			...
			if (use_dropout)
				layers.push_back ( new Dropout ( layer_sizes[l], layer_sizes[l], batch_size, 0.5f ) );
