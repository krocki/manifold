...
* @Last Modified time: 2017-04-06 16:23:31
*/
...
class CIFARImporter {
	...
	public:
		...
		static std::deque<datapoint> importFromFiles ( std::initializer_list<std::string> files,
				size_t label_bytes = C10_LABEL_BYTES, int filter = -1 ) {
			...
			for ( std::string filename : files ) {
				...
				if ( infile.is_open() ) {
					...
					std::cout << "Loaded  " << out.size() << " points" << std::endl;
