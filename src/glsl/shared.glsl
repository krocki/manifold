...
vec4 apply_colormap ( vec3 val, int colormap, float alpha, int discretize ) {
	...
	switch ( colormap ) {
		...
		case COLORMAP_GRAY: return vec4 ( gray ( ( val.r + val.g + val.b ) / 3.0f, discretize ), alpha );
		case COLORMAP_JET: return vec4 ( jet ( ( val.r + val.g + val.b ) / 3.0f, discretize ), alpha );
		case COLORMAP_PARULA: return vec4 ( parula ( ( val.r + val.g + val.b ) / 3.0f, discretize ), alpha );
		case COLORMAP_VIRIDIS: return vec4 ( viridis ( ( val.r + val.g + val.b ) / 3.0f, discretize ), alpha );
		case COLORMAP_RGB: return vec4 ( spectrum ( ( val.r + val.g + val.b ) / 3.0f, discretize ), alpha );
		case COLORMAP_HAXBY: return vec4 ( haxby ( ( val.r + val.g + val.b ) / 3.0f, discretize ), alpha );
		case COLORMAP_SEISMIC: return vec4 ( seismic ( ( val.r + val.g + val.b ) / 3.0f, discretize ), alpha );
