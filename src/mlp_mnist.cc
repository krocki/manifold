...
class MLPScreen : public nanogui::Screen {
  ...
  public:
	...
	void init() {
		...
		*/
		...
		nn_window->setPosition({340, 35 });
		nn_imgPanel = new nanogui::ImagePanel(nn_window, 36, 2, 4, {16, 16});
	...
	virtual bool keyboardEvent ( int key, int scancode, int action, int modifiers ) {
		...
		if ( key == GLFW_KEY_SLASH && action == GLFW_PRESS) {
			...
			//full view
			console_mode = false; data_mode = false; mini_mode = false;
			setSize(prev_size);
		...
		if ( key == GLFW_KEY_PERIOD && action == GLFW_PRESS) {
			...
			//console view
			console_mode = true; data_mode = false; mini_mode = false;
		...
		if ( key == GLFW_KEY_COMMA && action == GLFW_PRESS) {
			...
			// mini view
			console_mode = false; data_mode = false; mini_mode = true;
		...
		if ( key == GLFW_KEY_M && action == GLFW_PRESS) {
			...
			// data view
			console_mode = false; mini_mode = false; data_mode = true;
			setSize({DATA_MODE_WIDTH, DEF_HEIGHT});
	...
	virtual bool resizeEvent ( const Eigen::Vector2i & size ) {
		...
		if (console_mode) {
			...
			if (!mini_mode && !console_mode && !data_mode)
				prev_size = size;
		}
