...
* @Last Modified time: 2017-04-05 22:54:35
*/
...
class PlotData {
  ...
  public:
	...
	void load_data_textures(std::deque<datapoint>& data, NVGcontext *nvg) {
		...
		data_textures = Texture(data, GL_RED, nvg);

	}

	void load_c100_data_textures(std::deque<datapoint>& data, NVGcontext *nvg) {

		c100_data_textures = Texture(data, GL_RGBA, nvg, 32);

	}

	void update_nn_matrix_textures(std::shared_ptr<NN>& net, NVGcontext *nvg) {
		...
		if (net) {
			...
			if (net->layers[0]) {
				...
				make_textures_from_matrices(nn_matrix_data[0], net->layers[0]->x, nvg, GL_RGBA, SQUARES);
				// weights layer 0
				make_textures_from_matrices(nn_weight_data[0], ((Linear*)net->layers[0])->W, nvg, GL_RGBA, SQUARES, true);
			}

			// reconstructions - output from the top layer
			if (net->layers.back())
				make_textures_from_matrices(nn_matrix_data.back(), net->layers.back()->y, nvg, GL_RGBA, SQUARES);
		} else {
	...
	void make_textures_from_matrices(Texture& t, Eigen::MatrixXf &m, NVGcontext *nvg, GLint fmt = GL_RED, matrix_layout layout = SQUARES, bool transpose = false) {
