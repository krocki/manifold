...
* @Last Modified time: 2017-04-20 20:05:13
*/
...
class PlotData {
	...
	public:
		...
		Eigen::MatrixXf p_vertices, p_colors, reconstr_data;
		Eigen::MatrixXf s_vertices, s_colors, s_reconstr_data;
		...
		std::vector<Eigen::MatrixXf> rgba_texture_data;
		...
		void gan_update_reconstructions ( std::shared_ptr<NN> &net, NVGcontext *nvg, size_t im_size, bool rgba = false ) {
		
			if ( net->layers.back() )
				make_textures_from_matrices ( input_reconstruction_textures, reconstr_data, nvg, rgba ? GL_RGBA : GL_RED, SQUARES );
				
			make_textures_from_matrices ( sample_reconstruction_textures, s_reconstr_data, nvg, rgba ? GL_RGBA : GL_RED, SQUARES );
		}
