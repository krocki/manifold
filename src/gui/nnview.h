...
* @Last Modified time: 2017-04-11 20:31:14
*/
  ...
  public:
	...
	void setnets ( std::shared_ptr<NN> _net, std::shared_ptr<NN> _disc ) {
		...
		if ( disc ) {
			...
			wwww =  new nanogui::Window ( nets, "" );
			wwww->setLayout( new nanogui::GridLayout ( nanogui::Orientation::Horizontal, disc->layers.size(), nanogui::Alignment::Middle,
			                 3, 3 ));
			...
			for ( int i = 0; i < disc->layers.size(); i++ ) {
				...
				nanogui::Label* l = new nanogui::Label ( wwww, std::to_string ( i ) + ": " + disc->layers[i]->name + ", x: "  + std::to_string ( disc->layers[i]->x.rows() ) + ":" +  std::to_string ( disc->layers[i]->x.cols() ) + ", y: "  +	std::to_string ( disc->layers[i]->y.rows() ) + ":" + std::to_string ( disc->layers[i]->y.cols() ), "sans-bold", 9);
				l->setFixedSize( { 40, 30});
			...
			for ( int i = 0; i < disc->layers.size(); i++ ) {
				...
				disc->layers[i]->x_avg_activity = avg_inputs[i]->values_ptr();
				...
				disc->layers.back()->x_hist_activity = hist_inputs[i]->values_ptr();
			...
			for ( int i = 0; i < disc->layers.size(); i++ ) {
				...
				disc->layers[i]->y_avg_activity = avg_outputs[i]->values_ptr();
				...
				disc->layers[i]->y_hist_activity = hist_outputs[i]->values_ptr();
			...
			for ( int i = 0; i < disc->layers.size(); i++ ) {
				...
				disc->layers[i]->dx_avg_activity = avg_dinputs[i]->values_ptr();
				...
				disc->layers[i]->dx_hist_activity = hist_dinputs[i]->values_ptr();
			...
			for ( int i = 0; i < disc->layers.size(); i++ ) {
				...
				disc->layers[i]->dy_avg_activity = avg_doutputs[i]->values_ptr();
				...
				disc->layers[i]->dy_hist_activity = hist_doutputs[i]->values_ptr();
