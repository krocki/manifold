...
* @Last Modified time: 2017-04-20 16:35:45
*/
...
class NNView : public nanogui::Window {
	...
	public:
		...
		NNView ( Widget *parent, NVGcontext *_nvg, PlotData *_plot_data, nanogui::Window *large_image_view,
				 nanogui::ImageView *large_tex_view, nanogui::ImageView *small_tex_view ) : nanogui::Window ( parent, "" ) {
		...
		void update_matrices() {
			...
			if ( disc ) {
				for ( int i = 0; i < disc->layers.size(); i++ ) {
					...
					imgPanel_disc[i]->setCallback ( [this, i] ( int k ) {
						...
						if ( popup_w && popup_i && popup_si ) {
							...
							popup_si->bindImage ( plot_data->disc_matrices[i][k].first );
							popup_w->setVisible ( true );
							popup_w->requestFocus();
				...
				std::cout << "discriminator is null" << std::endl;
		...
		void setnets ( std::shared_ptr<NN> _net, std::shared_ptr<NN> _disc ) {
			...
			if ( net ) {
				...
				for ( int i = 0; i < net->layers.size(); i++ )
					...
					nanogui::Label *l = new nanogui::Label ( wwww, std::to_string ( i ) + ": " + net->layers[i]->name + ", x: "  + std::to_string ( net->layers[i]->x.rows() ) + ":" + std::to_string (
																 net->layers[i]->x.cols() ) + ", y: "  + std::to_string ( net->layers[i]->y.rows() ) + ":" + std::to_string ( net->layers[i]->y.cols() ), "sans-bold", 9 );
		...
		nanogui::ImageView *popup_i, *popup_si;
