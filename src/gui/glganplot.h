...
* @Last Modified time: 2017-04-20 13:59:45
*/
...
#define POINT_SHADER_NAME "gan_datapoint_shader"
#define POINT_FRAG_FILE "./src/glsl/gan_datapoint.f.glsl"
#define POINT_VERT_FILE "./src/glsl/gan_datapoint.v.glsl"

#define POINT_TEX_SHADER_NAME "gan_datapoint_tex_shader"
#define POINT_TEX_FRAG_FILE "./src/glsl/gan_tex_point.f.glsl"
...
class Plot : public nanogui::GLCanvas {
	...
	public:
		...
		Plot ( Widget *parent, std::string _caption, const Eigen::Vector2i &w_size, int i, PlotData *plot_data,
			   ...
			   const std::string r_prefix = "", int blendmode = 0, bool depth_test = false, bool enabled = true ) : nanogui::GLCanvas ( parent, transparent ) {
			...
			b->setBackgroundColor ( nanogui::Color ( 192, 128, 0, 96 ) );
			...
			b = shader_tools->add<nanogui::Button> ( "", ENTYPO_ICON_TWO_DOTS );
			b->setFlags ( nanogui::Button::ToggleButton );
			b->setFixedSize ( bsize );
			b->setBackgroundColor ( nanogui::Color ( 192, 128, 0, 96 ) );
			...
			b = this->add<nanogui::Button> ( "", ENTYPO_ICON_BLOCK );
			b->setFlags ( nanogui::Button::ToggleButton );
			b->setFixedSize ( bsize );
			b->setPosition ( {3, 3} );
			b->setBackgroundColor ( ccolor );
			b->setPushed ( !on );
			b->setChangeCallback ( [&] ( bool state ) { on = !on; } );
			
			nanogui::Window *palettes = new nanogui::Window ( this );
			b = palettes->add<nanogui::Button> ( "", ENTYPO_ICON_PALETTE );
			b->setFixedSize ( bsize );
			palettes->setFixedSize ( bsize );
			palettes->setPosition ( {3, bsize[1] + 5} );
			b->setBackgroundColor ( nanogui::Color ( 255, 120, 0, 64 ) );
			
			nanogui::ComboBox *cb = new nanogui::ComboBox ( b, { available_colormaps } );
			cb->setFontSize ( 12 );
			cb->setCallback ( [&] ( const int &value ) { std::cout << "selected colormap: " << value << std::endl; selected_colormap = value; } );
			
			nanogui::Window *blendmodes = new nanogui::Window ( this );
			b = blendmodes->add<nanogui::Button> ( "", ENTYPO_ICON_LIGHT_BULB );
			b->setFixedSize ( bsize );
			blendmodes->setFixedSize ( bsize );
			blendmodes->setPosition ( {3, 2 * ( bsize[1] + 5 ) } );
			b->setBackgroundColor ( nanogui::Color ( 225, 130, 0, 64 ) );
			
			nanogui::ComboBox *cb2 = new nanogui::ComboBox ( b, { available_blendmodes } );
			cb2->setFontSize ( 12 );
			cb2->setCallback ( [&] ( const int &value ) { std::cout << "selected blendmode: " << value << std::endl; selected_blendmode = value; } );
			
			b = this->add<nanogui::Button> ( "", ENTYPO_ICON_INFINITY );
			b->setFlags ( nanogui::Button::ToggleButton );
			b->setFixedSize ( bsize );
			b->setPosition ( {3, 3 * ( bsize[1] + 5 ) } );
			b->setBackgroundColor ( nanogui::Color ( 225, 130, 0, 64 ) );
			b->setPushed ( depth_test_enabled );
			b->setChangeCallback ( [&] ( bool state ) { depth_test_enabled = !depth_test_enabled; } );
			
			b = this->add<nanogui::Button> ( "", ENTYPO_ICON_CODE );
			b->setFlags ( nanogui::Button::ToggleButton );
			b->setFixedSize ( bsize );
		...
		void init_shaders() {
			...
			m_pointShader->initFromFiles ( POINT_SHADER_NAME, POINT_VERT_FILE, POINT_FRAG_FILE, "", SHARED_GLSL_FILE );
			...
			m_pointTexShader->initFromFiles ( POINT_TEX_SHADER_NAME, POINT_TEX_VERT_FILE, POINT_TEX_FRAG_FILE,
											  POINT_TEX_GEOM_FILE, SHARED_GLSL_FILE );
											  
			m_samplePointShader = new nanogui::GLShader();
			m_samplePointShader->initFromFiles ( POINT_SHADER_NAME, POINT_VERT_FILE, POINT_FRAG_FILE, "", SHARED_GLSL_FILE );
			...
			m_cubeShader->initFromFiles ( BOX_SHADER_NAME, BOX_VERT_FILE, BOX_FRAG_FILE, BOX_GEOM_FILE, SHARED_GLSL_FILE );
			
			m_camShader = new nanogui::GLShader();
			m_camShader->initFromFiles ( CAM_SHADER_NAME, CAM_VERT_FILE, CAM_FRAG_FILE, CAM_GEOM_FILE, SHARED_GLSL_FILE );
			
			m_rayShader = new nanogui::GLShader();
			m_rayShader->initFromFiles ( RAY_SHADER_NAME, RAY_VERT_FILE, RAY_FRAG_FILE, RAY_GEOM_FILE, SHARED_GLSL_FILE );
			
			m_meshShader = new nanogui::GLShader();
			m_meshShader->initFromFiles ( MESH_SHADER_NAME, MESH_VERT_FILE, MESH_FRAG_FILE, MESH_GEOM_FILE, SHARED_GLSL_FILE );
		...
		void refresh_data() {
			...
			if ( data ) {
				...
				m_pointTexShader->uploadAttrib ( "texcoords", data->input_reconstruction_textures.p_texcoords );
		...
		void drawGL() override {
			...
			if ( on ) {
				...
				switch ( selected_blendmode ) {
					...
					case 3: glDisable ( GL_BLEND ); break;
				...
				if ( show_data_points ) {
					...
					if ( !use_textures ) {
						...
						float textures_per_dim = data->input_reconstruction_textures.txs_per_dim;
						// std::cout << textures_per_dim << std::endl;
						...
						float quad_size = 0.005f;
						float radius = sqrtf ( 2 * quad_size );
						float tex_w = 1.0f / ( float ) ( data->input_reconstruction_textures.txs_per_dim );
				...
				if ( vg ) {
					...
					nvgFontBlur ( vg, 0.3f );
					nvgFillColor ( vg, nanogui::Color ( 1.0f, 1.0f, 1.0f, 0.5f ) );
					nvgText ( vg, mPos.x() + size().x() - 78, mPos.y() + size().y() - 3, string_format ( "%010d, %.1f fps",
																										 num_frames, 1.0f / frame_time ).c_str(), nullptr );
					...
					// nvgFontBlur ( vg, 0.3f );
					// nvgFillColor ( vg, nanogui::Color ( 1.0f, 1.0f, 1.0f, 0.5f ) );
					// nvgText ( vg, mPos.x() + size().x() - 28, mPos.y() + size().y() - 13, string_format ( "%d", num_frames ).c_str(), nullptr );
		...
		int selected_blendmode = 0;
		bool discretize_colormaps = 1;
