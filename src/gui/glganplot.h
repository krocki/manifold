...
* @Last Modified time: 2017-04-18 09:37:18
*/
  ...
  public:
	...
	void refresh_data() {
		...
		if ( data ) {
			...
			m_samplePointShader->uploadAttrib ( "position", data->p_vertices );
			m_samplePointShader->uploadAttrib ( "color", data->p_colors );

			m_samplePointTexShader->bind();
			m_samplePointTexShader->uploadAttrib ( "position", data->p_vertices );
			m_samplePointTexShader->uploadAttrib ( "color", data->p_colors );
			m_samplePointTexShader->uploadAttrib ( "texcoords", data->input_reconstruction_textures.p_texcoords );
	...
	void drawGL() override {
		...
		if (on) {
			...
			if ( show_samples && data->s_vertices.cols() > 0 ) {
				...
				m_samplePointShader->drawArray ( GL_POINTS, 0, data->p_vertices.cols() );
				...
				m_samplePointTexShader->drawArray ( GL_POINTS, 0, data->p_vertices.cols() );
