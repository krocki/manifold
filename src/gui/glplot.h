...
* @Last Modified time: 2017-03-31 11:50:11
*/
...
class Plot : public nanogui::GLCanvas {
  ...
  public:
	...
	Plot ( Widget *parent, std::string _caption, const Eigen::Vector2i &w_size, int i, PlotData *plot_data,
	       bool transparent = false, GLFWwindow *w = nullptr, NVGcontext *nvg = nullptr, float _fovy = 67.0f,
	       const Eigen::Vector3f _camera = Eigen::Vector3f ( 0.0f, 0.0f, 5.0f ),
	       const Eigen::Vector3f _rotation = Eigen::Vector3f ( 0.0f, 0.0f, 0.0f ),
	       bool _ortho = false ) : nanogui::GLCanvas ( parent, transparent ) {
	...
	void init_shaders() {
		...
		Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic> rgba_image =
		    Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Ones ( tex_size, tex_size * 4 ) * 16;
		...
		textures.emplace_back ( std::pair<int, std::string> ( nvgCreateImageRGBA ( vg,  tex_size, tex_size, NVG_IMAGE_NEAREST,
