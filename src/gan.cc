...
* @Last Modified time: 2017-04-13 16:18:13
*/
...
int compute() {
	...
	discriminator = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, MLP, {input_width, 25, 1}, SIGMOID ) );
	...
	while ( screen->getVisible() ) {
		...
		// generate ( uniform_real_distribution<> ( 0, 1 ), uniform_real_distribution<> ( 0, 1 ),
		//            uniform_real_distribution<> ( 0, 1 ), gan_train_data.noise.x, batch_size, INDEPENDENT );


		generate ( std::normal_distribution<> ( 0, 1 ), std::normal_distribution<> ( 0, 1 ),
		           std::normal_distribution<> ( 0, 1 ), gan_train_data.noise_n.x, batch_size, INDEPENDENT );
		...
		smooth_discriminator_real_acc = smooth_discriminator_real_acc < 0 ? discriminator_real_acc : 0.99 *
		...
		smooth_discriminator_fake_acc = smooth_discriminator_fake_acc < 0 ? discriminator_fake_acc : 0.99 *
