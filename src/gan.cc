...
* @Last Modified time: 2017-04-13 19:51:08
*/
...
int compute() {
	...
	nn = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, AE, {3, 64, input_width }, RELU ) );
	...
	while ( screen->getVisible() ) {
		...
		generate ( std::normal_distribution<> ( 0, 0.5 ), std::normal_distribution<> ( 0, 0.5 ),
				   std::normal_distribution<> ( 0, 0.5 ), gan_train_data.noise_n.x, batch_size, INDEPENDENT );
