...
* @Last Modified time: 2017-04-18 21:06:07
*/
...
int compute() {
	...
	discriminator = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, MLP, {input_width, 64, 64, 1}, RELU, true ) );
	encoder = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, MLP, {input_width, 64, 64}, RELU, true ) );
	...
	if ( screen )
		if ( screen->nnview )
			screen->nnview->setnets ( nn, encoder );
