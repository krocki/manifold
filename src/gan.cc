...
* @Last Modified time: 2017-04-19 13:28:32
*/
...
int compute() {
	...
	size_t code_dims = 3;
	bool label_smoothing = true;
	
	nn = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, AE, {static_cast<int> ( code_dims ), 256, input_width }, RELU ) );
	discriminator = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, MLP, {input_width, 256, 1}, RELU, false ) );
	encoder = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, MLP, {input_width, 256 }, RELU, false ) );
	...
	while ( screen->getVisible() ) {
		...
		generate_ndims ( code_dims, std::normal_distribution<> ( 0, 30 ), gan_train_data.noise.x, batch_size, INDEPENDENT );
		...
		gl_data->s_colors.block ( 1, batch_size * batch_iter, 1, batch_size ) = ( gan_train_data.mix_indices.cast<float>().array() ) * 1;
		gl_data->s_colors.block ( 2, batch_size * batch_iter, 1, batch_size ) = ( gan_train_data.mix_indices.cast<float>().array() ) * 1;
		gl_data->s_colors.block ( 0, batch_size * batch_iter, 1, batch_size ) = ( gan_train_data.mix_indices.cast<float>().array() ) * 1;
		...
		if ( label_smoothing ) gan_train_data.mixed.y = gan_train_data.mixed.y.array() * 0.9f;
