...
* @Last Modified time: 2017-04-21 18:59:55
*/
...
int compute() {
	...
	discriminator = std::shared_ptr<NN> ( new NN ( batch_size, decay, learning_rate, MLP, {input_width, 256, 64, 1}, RELU, false ) );
	...
	discriminator->label_smoothing = 0.8f;
	...
	while ( screen->getVisible() ) {
		...
		if ( nn->generator_loss_type == NON_SATURATING_LOSS ) {
			...
			nn->backward ( discriminator->layers[0]->dx.array(), accumulate_grads );
